...
openssl req -x509 -new -key elastic-root-ca.key -days 3650 -nodes -subj "/CN=production-deployment CA" -sha256 -out elastic-root-ca.pem
...
openssl x509 -req -in elastic-node.csr -CA elastic-root-ca.pem -CAkey elastic-root-ca.key -CAcreateserial -days 3650 -out elastic-node.pem -sha256
